/**
 * POS Self-custodial Payment E2E test.
 *
 * Exercises the crypto self-custodial payment flow end-to-end:
 ...
 *   6. Derive customer's UTXO key: password → sign → SHA-256
 *   7. Derive customer's P256 keypairs from UTXO key
 *   8. Build ONE atomic bundle:
 *      - DEPOSIT (customer signs, conditioned on customer CREATEs)
 *      - customer CREATEs (at customer's derived P256 keys)
 *      - SPENDs from customer keys (signed with P256, conditioned on merchant CREATEs)
 *      - merchant CREATEs (at merchant receive UTXOs)
 *   9. Submit via pay-platform /pay/instant/submit
 *  10. Verify transaction recorded + merchant balance updated
 *
 * This mirrors the flow in moonlight-pay/src/lib/selfcustodial-payment.ts
...
const DEPOSIT_FEE_STROOPS = 500_000n;
const CUSTOMER_PASSWORD = "test-self-custodial-password-2026";
...
/** password → sign with ed25519 → SHA-256 → 32-byte UTXO derivation key */
async function deriveUtxoKey(password: string, kp: Keypair): Promise<Uint8Array> {
  const sigBytes = kp.sign(Buffer.from(password));
  const buf = new ArrayBuffer(sigBytes.length);
  new Uint8Array(buf).set(sigBytes);
  return new Uint8Array(await crypto.subtle.digest("SHA-256", buf));
}

/** Derive N P256 keypairs from a UTXO derivation key + index */
async function deriveUtxoKeypairs(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const seed = new Uint8Array(await crypto.subtle.digest("SHA-256", seedInput));
    keypairs.push(await deriveP256Keypair(seed));
  }
  return keypairs;
}
...
const createRes = await payApi("/account", {
  ...
  body: JSON.stringify({
    email: "merchant-sc@test.local",
    jurisdictionCountryCode: "US",
    displayName: "Test Merchant SC",
  }),
});
...
console.log("\n[3/10] Generating and storing merchant receive UTXOs...");
const merchantUtxoPayloads = [];
for (let i = 0; i < 5; i++) {
  const kp = await deriveP256Keypair(crypto.getRandomValues(new Uint8Array(32)));
...
if (!prepareRes.ok) throw new Error(`Prepare failed: ${prepareRes.status} ${await prepareRes.text()}`);
...
console.log("\n[6/10] Authenticating customer with provider-platform...");
const providerJwt = await authenticateWithProvider(customer);
console.log(`  Authenticated (${elapsed()})`);

// [7] Derive customer UTXO key from password signature
console.log("\n[7/10] Deriving UTXO key from password...");
const utxoKey = await deriveUtxoKey(CUSTOMER_PASSWORD, customer);
console.log(`  UTXO key derived (${elapsed()})`);

// [8] Derive customer P256 keypairs + build bundle
console.log("\n[8/10] Building self-custodial payment bundle...");
const ledgerSequence = await getLatestLedger();
const expiration = ledgerSequence + 1000;

const customerKeypairCount = 5;
const customerKeypairs = await deriveUtxoKeypairs(utxoKey, customerKeypairCount);
...
// Customer CREATE ops (at customer's password-derived P256 keys)
const customerAmounts = partitionAmount(PAYMENT_STROOPS, customerKeypairCount);
const customerCreateOps = customerKeypairs.map((kp, i) =>
  MoonlightOperation.create(kp.publicKey, customerAmounts[i])
);
...
// SPEND ops — from customer's derived keys, conditioned on merchant CREATEs
const spendOps = [];
for (let i = 0; i < customerKeypairs.length; i++) {
  const spendOp = MoonlightOperation.spend(customerKeypairs[i].publicKey);
  for (const merchantCreate of merchantCreateOps) {
    spendOp.addCondition(merchantCreate.toCondition());
  }
  await spendOp.signWithUTXO(
    makeUtxoAdapter(customerKeypairs[i]),
    privacyChannelId as ContractId,
    expiration,
  );
...
// Assemble: DEPOSIT, customer CREATEs, SPENDs, merchant CREATEs
const operationsMLXDR = [
  depositOp.toMLXDR(),
  ...customerCreateOps.map((op) => op.toMLXDR()),
...
// [9] Submit via pay-platform
console.log("\n[9/10] Submitting via pay-platform...");
const submitRes = await payApi("/pay/instant/submit", {
  ...
  body: JSON.stringify({
    ...
    description: "POS self-custodial e2e test",
    operationsMLXDR,
...
console.log(`\n✅ POS self-custodial payment test passed in ${elapsed()}`);
