 ...
 *
 * Calls the actual moonlight-pay `executeSelfCustodialPayment` function —
 * the same code path the browser UI uses. The signer and signMessage are
 * raw-keypair mocks instead of Freighter, producing the same output.
 */
...
import { executeSelfCustodialPayment } from "./moonlight-pay-lib/selfcustodial-payment.ts";
import { __resetConfigForTests } from "./moonlight-pay-lib/config.ts";
import {
  createTestSignMessage,
  createTestSigner,
  fundAccount,
...
// [4] Execute self-custodial payment via the actual moonlight-pay function
console.log("\n[4/5] Executing self-custodial payment...");
const signer = createTestSigner(customer, NETWORK_PASSPHRASE);
const signMessage = createTestSignMessage(customer);
