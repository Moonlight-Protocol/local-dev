// deno-lint-ignore-file no-explicit-any -- e2e harness inspects raw Soroban returns whose runtime shape is dynamic
import {
  Address,
  Contract,
  Keypair,
...
const COUNCIL_SK = Deno.env.get("E2E_COUNCIL_SK") ??
  Deno.env.get("COUNCIL_SK") ?? councilEnv["COUNCIL_SK"]!;
const AUTH_ID = Deno.env.get("E2E_CHANNEL_AUTH_ID") ??
  Deno.env.get("CHANNEL_AUTH_ID") ?? councilEnv["CHANNEL_AUTH_ID"]!;
const CHANNEL_ID = Deno.env.get("E2E_CHANNEL_CONTRACT_ID") ??
  Deno.env.get("CHANNEL_CONTRACT_ID") ?? providerEnv["CHANNEL_CONTRACT_ID"]!;
...
async function addProviderOnChain(
  adminKp: Keypair,
  ...
  if ("error" in sim && sim.error) {
    throw new Error(`add_provider simulation failed: ${sim.error}`);
  }
...
async function waitForMembershipStatus(
  ppPublicKey: string,
...
await post(COUNCIL_API, `/council/jurisdictions?councilId=${AUTH_ID}`, {
  countryCode: "UY",
  label: "Uruguay",
}, councilToken);
await post(COUNCIL_API, `/council/channels?councilId=${AUTH_ID}`, {
...
await post(PROVIDER_API, "/dashboard/pp/register", {
  secretKey: pp1.secret(),
  derivationIndex: 0,
  label: "PP-Approve",
}, dashToken);

const j1Envelope = await signJoinPayload({
  publicKey: pp1.publicKey(),
  councilId: AUTH_ID,
  label: "PP-Approve",
  contactEmail: "a@t",
  jurisdictions: null,
...
const ms1pre = await fetch(
  `${COUNCIL_API}/public/provider/membership-status?councilId=${AUTH_ID}&publicKey=${pp1.publicKey()}`,
);
check("Membership-status: PENDING (202)", ms1pre.status, 202);

const p1 = await get(
  COUNCIL_API,
  `/council/provider-requests?councilId=${AUTH_ID}&status=PENDING`,
  councilToken,
);
...
const a1 = await post(
  COUNCIL_API,
  `/council/provider-requests/${r1id}/approve`,
  {},
  councilToken,
);
...
const cr1 = await get(
  COUNCIL_API,
  `/council/provider-requests?councilId=${AUTH_ID}`,
  councilToken,
);
const cr1entry = cr1.data.data?.find((r: any) =>
  r.publicKey === pp1.publicKey()
);
...
const ewActivated = await waitForMembershipStatus(
  pp1.publicKey(),
  "ACTIVE",
  dashToken,
);
...
await post(PROVIDER_API, "/dashboard/pp/register", {
  secretKey: pp2.secret(),
  derivationIndex: 1,
  label: "PP-Reject",
}, dashToken);

const j2Envelope = await signJoinPayload({
  publicKey: pp2.publicKey(),
  councilId: AUTH_ID,
  label: "PP-Reject",
  contactEmail: "r@t",
  jurisdictions: null,
...
const rej = await post(
  COUNCIL_API,
  `/council/provider-requests/${r2id}/reject`,
  {},
  councilToken,
);
...
const cr2entry = cr2.data.data?.find((r: any) =>
  r.publicKey === pp2.publicKey()
);
...
const ms2 = await fetch(
  `${COUNCIL_API}/public/provider/membership-status?councilId=${AUTH_ID}&publicKey=${pp2.publicKey()}`,
);
...
await post(PROVIDER_API, "/dashboard/pp/register", {
  secretKey: pp6.secret(),
  derivationIndex: 3,
  label: "PP-Dup",
}, dashToken);

const j6aEnvelope = await signJoinPayload({
  publicKey: pp6.publicKey(),
  councilId: AUTH_ID,
  label: "PP-Dup",
  contactEmail: "dup@t",
  jurisdictions: null,
...
// Unknown PP → 404
const msUnknown = await fetch(
  `${COUNCIL_API}/public/provider/membership-status?councilId=${AUTH_ID}&publicKey=${Keypair.random().publicKey()}`,
);
...
await post(PROVIDER_API, "/dashboard/pp/register", {
  secretKey: pp8a.secret(),
  derivationIndex: 4,
  label: "PP-Multi-A",
}, dashToken);
await post(PROVIDER_API, "/dashboard/pp/register", {
  secretKey: pp8b.secret(),
  derivationIndex: 5,
  label: "PP-Multi-B",
}, dashToken);

const j8aEnvelope = await signJoinPayload({
  publicKey: pp8a.publicKey(),
  councilId: AUTH_ID,
  label: "PP-Multi-A",
  contactEmail: null,
  jurisdictions: null,
...
// Approve both
const pendingMulti = await get(
  COUNCIL_API,
  `/council/provider-requests?councilId=${AUTH_ID}&status=PENDING`,
  councilToken,
);
const multiReqs = pendingMulti.data.data?.filter((r: any) =>
  r.label === "PP-Multi-A" || r.label === "PP-Multi-B"
);
...
for (const req of multiReqs || []) {
  await post(
    COUNCIL_API,
    `/council/provider-requests/${req.id}/approve`,
    {},
    councilToken,
  );
}
...
const ms8a = await fetch(
  `${COUNCIL_API}/public/provider/membership-status?councilId=${AUTH_ID}&publicKey=${pp8a.publicKey()}`,
);
const ms8b = await fetch(
  `${COUNCIL_API}/public/provider/membership-status?councilId=${AUTH_ID}&publicKey=${pp8b.publicKey()}`,
);
check("PP-Multi-A ACTIVE", ms8a.status, 200);
check("PP-Multi-B ACTIVE", ms8b.status, 200);

// Both show in council
const allProviders = await get(
  COUNCIL_API,
  `/council/provider-requests?councilId=${AUTH_ID}`,
  councilToken,
);
const multiA = allProviders.data.data?.find((r: any) =>
  r.label === "PP-Multi-A"
);
const multiB = allProviders.data.data?.find((r: any) =>
  r.label === "PP-Multi-B"
);
...
await post(COUNCIL_API, `/council/jurisdictions?councilId=${council2Id}`, {
  countryCode: "BR",
  label: "Brazil",
}, councilToken);
...
await post(PROVIDER_API, "/dashboard/pp/register", {
  secretKey: pp9.secret(),
  derivationIndex: 6,
  label: "PP-Council2",
}, dashToken);

const j9Envelope = await signJoinPayload({
  publicKey: pp9.publicKey(),
  councilId: council2Id,
  label: "PP-Council2",
  contactEmail: null,
  jurisdictions: null,
...
const c2reqs = await get(
  COUNCIL_API,
  `/council/provider-requests?councilId=${council2Id}`,
  councilToken,
);
...
console.log(
  `\n\x1b[34m=== ${
    failures === 0 ? "\x1b[32mALL PASSED" : `\x1b[31m${failures} FAILED`
  } \x1b[34m===\x1b[0m\n`,
);
