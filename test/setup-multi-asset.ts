/**
 * Multi-asset (UC6) test setup — writes minimal platform config only.
 *
 * Like the lifecycle test, the multi-asset test handles ALL deployment and
 * seeding itself (two channels under one auth). Setup only writes the bare
 * .env files so provider and council can start.
 */
...
async function main() {
  console.log("[setup-multi-asset] Writing minimal config...");
...
`;
  ...
  console.log(`[setup-multi-asset] Config written to ${CONFIG_DIR}/`);
  console.log("[setup-multi-asset] Done.");
}

main().catch((err) => {
  console.error("[setup-multi-asset] FAILED:", err);
  Deno.exit(1);
});
