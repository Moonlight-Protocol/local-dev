...
const LOCAL_URLS: ServiceUrls = {
  providerConsole: process.env.PROVIDER_CONSOLE_URL ?? `http://localhost:${process.env.PROVIDER_CONSOLE_PORT ?? "3020"}`,
  councilConsole: process.env.COUNCIL_CONSOLE_URL ?? `http://localhost:${process.env.COUNCIL_CONSOLE_PORT ?? "3030"}`,
  moonlightPay: process.env.MOONLIGHT_PAY_URL ?? `http://localhost:${process.env.MOONLIGHT_PAY_PORT ?? "3050"}`,
  networkDashboard: process.env.NETWORK_DASHBOARD_URL ?? `http://localhost:${process.env.NETWORK_DASHBOARD_PORT ?? "3040"}`,
  providerApi: process.env.PROVIDER_API_URL ?? `http://localhost:${process.env.PROVIDER_PORT ?? "3010"}`,
  councilApi: process.env.COUNCIL_API_URL ?? `http://localhost:${process.env.COUNCIL_PLATFORM_PORT ?? "3015"}`,
  payApi: process.env.PAY_API_URL ?? `http://localhost:${process.env.PAY_PLATFORM_PORT ?? "3025"}`,
};
...
export function getJaegerUrl(): string {
  return process.env.JAEGER_QUERY_URL ?? `http://localhost:${process.env.JAEGER_UI_PORT ?? "16686"}`;
}
